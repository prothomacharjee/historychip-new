...
// Story Comments
    ...
    // SOLR SERCH ENGINE
    public static function fullTextSearchIds($term)
    {
        $solrHost = env('SOLR_HOST', '3.91.8.252');
        $solrPort = env('SOLR_PORT', '8983');
        $solrRows = env('SOLR_PAGE_SIZE', 50);
        $cleanedTerm = preg_replace('/\\s+/', '||', $term);
        $urlTerm = urlencode($cleanedTerm);
        $queryUrl = "http://$solrHost:$solrPort/solr/historychip/select?fl=id&q=author:%22$urlTerm%22%20OR%20context:%22$urlTerm%22OR%20title:%22$urlTerm%22&wt=phps&rows=$solrRows";
        $storyIds = self::solrSearch($queryUrl);
        return $storyIds;
    }

    public static function solrSearch($queryUrl)
    {
        $serializedResult = file_get_contents($queryUrl);
    ...
    public function searchBasic(Request $request)
    {
        ...
        $authors = Story::getauthors();

        $test = $request->input('search', 'history');

        $storyIds = $this->fullTextSearchIds($request->input('term', 'history'));

        $storiesData = Story::storiesById($storyIds, $sortBy);


        return view("readastory")->with(["authors" => $authors, 'stories' => $paginatedItems, 'command' => 'search', 'title' => $title, 'admin' => $this->admin, 'searchTerm' => $test, 'sort_by' => $sortBy]);
        //return view('storypreviews')->with(['stories' => $storiesData, 'command' => 'search']);
    }
    ...
    public function searchByAuthor(Request $request)
    {
        ...
        $authors = Story::getauthors();
        $storiesData = Story::getListOfStoriesByAuthor($request->input('slct', 'history'), $sortBy, 1);

        // Get current page form url e.x. &page=1
        $currentPage = LengthAwarePaginator::resolveCurrentPage();

        // Create a new Laravel collection from the array data
        $itemCollection = collect($storiesData);
        ...
        //return view("readastory")->with(["authors" => $authors, 'stories' => $storiesData::paginate(8), 'command' => 'search', 'title' => $title, 'admin' => $this->admin]);
        return view("readastory")->with(["authors" => $authors, 'stories' => $paginatedItems, 'command' => 'search', 'title' => $title, 'admin' => $this->admin, 'sort_by' => $sortBy]);
        //return view("readastory")->with(["authors" => $authors, 'stories' => compact('data'), 'command' => 'search', 'title' => $title, 'admin' => $this->admin]);
    }
