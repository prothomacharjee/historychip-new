...
class SoftSourceHelper
{
                    ...
                    if (in_array(strtolower($ext), $valid_formats)) {
                        ...
                        if ($size < (10 * 1024 * 1024)) {
                            ...
                            if (Storage::disk('public')->putFileAs($path, $tmp, $actual_image_name) != false) {
                                ...
                                header("HTTP/1.1 500");
                                if ($_FILES["story_header_image"]["error"] == 1) {
                                    $error = "The uploaded file exceeds the upload_max_filesize";
                                } else if ($_FILES["story_header_image"]["error"] == 2) {
                                    $error = "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.";
                                } else if ($_FILES["story_header_image"]["error"] == 3) {
                                    $error = "The uploaded file was only partially uploaded.";
                                } else if ($_FILES["story_header_image"]["error"] == 4) {
                                    $error = "No file was uploaded.";
                                } else if ($_FILES["story_header_image"]["error"] == 6) {
                                    $error = "Missing a temporary folder.";
                                } else if ($_FILES["story_header_image"]["error"] == 7) {
                                    $error = "Failed to write file to disk.";
                                } else if ($_FILES["story_header_image"]["error"] == 1) {
    ...
    public static function SaveAudioBYFileUploader($request) {
        $path = 'frontend/stories/audio-videos/';
        // $dir = realpath($path);
        $valid_formats = array("mp3", "mp4");
        if (isset($_FILES['story_audio_video']['name'])) {
            $name = $_FILES['story_audio_video']['name'];
            $size = $_FILES['story_audio_video']['size'];
            if (strlen($name)) {
                ...
                if (in_array(strtolower($ext), $valid_formats)) {
                    if ($size < (1000 * (1024 * 1024))) {
                        ...
                        if (Storage::disk('public')->putFileAs($path, $tmp, $actual_image_name) != false) {
                            ...
                            header("HTTP/1.1 500");
                            if ($_FILES["story_audio_video"]["error"] == 1) {
                                $error = "The uploaded file exceeds the upload_max_filesize";
                            } else if ($_FILES["story_audio_video"]["error"] == 2) {
                                $error = "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.";
                            } else if ($_FILES["story_audio_video"]["error"] == 3) {
                                $error = "The uploaded file was only partially uploaded.";
                            } else if ($_FILES["story_audio_video"]["error"] == 4) {
                                $error = "No file was uploaded.";
                            } else if ($_FILES["story_audio_video"]["error"] == 6) {
                                $error = "Missing a temporary folder.";
                            } else if ($_FILES["story_audio_video"]["error"] == 7) {
                                $error = "Failed to write file to disk.";
                            } else if ($_FILES["story_audio_video"]["error"] == 1) {
                                $error = "A PHP extension stopped the file upload. PHP does not provide a way to ascertain which extension caused the file upload to stop;";
                            } else {
                                $error = "Error";
                            }
