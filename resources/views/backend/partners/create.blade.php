                    ...
                    <div class="card-body">
                        <div class="p-4 border rounded">
                            <form class="row g-3 needs-validation" method="post" novalidate
                                action="{{ route('admin.partners.store') }}" id="partner_form"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="partner[id]" id="id"
                                    value="{{ old('id') ? old('id') : (!empty($partner->id) ? $partner->id : null) }}" />

                                <div class="col-md-6">
                                    <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                                    <input required minlength="5" maxlength="200" id="name" name="partner[name]"
                                        placeholder="Enter A Unique Partner Name Here" type="text"
                                        class="form-control @error('name') is-invalid @enderror"
                                        value="{{ old('name') ? old('name') : (!empty($partner->name) ? $partner->name : null) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input required minlength="5" maxlength="200" id="email" name="partner[email]"
                                        placeholder="Enter A Partner's Contact Email Here" type="text"
                                        class="form-control @error('email') is-invalid @enderror"
                                        value="{{ old('email') ? old('email') : (!empty($partner->email) ? $partner->email : null) }}">
                                    <div class="valid-feedback">Looks good!</div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control file-uploader" name="banner" id="banner" type="file"
                                        accept=".png,.jpg,.jpeg,.svg, .webp" maxFileSize="2MB"
                                ...
                                <div class="col-md-6">
                                    <label for="banner_alt_text" class="form-label">Banner Alt Text <span
                                            class="text-danger">*</span></label>
                                    <input minlength="3" maxlength="50" id="banner_alt_text"
                                        name="partner[banner_alt_text]" placeholder="Enter Banner Alter Text Here"
                                        type="text" class="form-control @error('banner_alt_text') is-invalid @enderror"
                                        value="{{ old('banner_alt_text') ? old('banner_alt_text') : (!empty($partner->banner_alt_text) ? $partner->banner_alt_text : null) }}">
                                    <div class="valid-feedback">Looks good!</div>
                                    ...
                                    <div class="selected-file-banner">
                                        @if (!empty($partner))
                                            @if (!empty($partner->banner))
                                                <img src="{{ $partner->banner }}" alt="{{ $partner->banner_alt_text }}"
                                                    class="w-25">
                                            @else
                                ...
                                <div class="col-md-12">
                                    <label for="partner_type_id" class="form-label">Partner Type <span
                                            class="text-danger">*</span></label>
                                    <select required name="partner[partner_type_id]" id="partner_type_id"
                                        class="form-control">
                                ...
                                <div class="col-md-12">
                                    <label for="title" class="form-label">Title <span
                                            class="text-danger">*</span></label>
                                    <input required minlength="5" maxlength="200" id="title" name="partner[title]"
                                        placeholder="Enter A Title For The Partner Page" type="text"
                                        class="form-control @error('title') is-invalid @enderror"
                                        value="{{ old('title') ? old('title') : (!empty($partner->title) ? $partner->title : null) }}">
                                    <div class="valid-feedback">Looks good!</div>
                                ...
                                <div class="col-md-8">
                                    ...
                                    <input class="form-control file-uploader" name="logo" id="logo"
                                        type="file" accept=".png,.jpg,.jpeg,.svg, .webp" maxFileSize="1MB"
                                        onchange="FileUploaderValidation(this, 1, ['image/png', 'image/jpeg', 'image/svg', 'image/webp'], 'logo', 25)" />
                                ...
                                <div class="col-md-12">
                                    <label for="description" class="form-label">Description <span
                                            class="text-danger">*</span></label>
                                    <textarea minlength="100" maxlength="500" rows="10"
                                        class="form-control @error('description') is-invalid @enderror" id="description" name="partner[description]"
                                        placeholder="Write Your Description Here" required aria-describedby="validationContentFeedback">{{ old('description') ? old('description') : (!empty($partner->description) ? $partner->description : null) }}</textarea>
                                    <div class="text-end partner-description-word-count">{{ $length }}/500</div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="file" id="partner_image" name="partner_image[]" multiple
                                        class="form-control" accept=".png,.jpg,.jpeg,.svg, .webp" maxFileSize="1MB"
                                        max="1" />
                                    <div id="file-error-partner-image" class="form-text text-danger"></div>
                                    <div class="selected-file-partner-image row">
                                        @if (!empty($partner))
                                            @if (!empty($partner->partner_image))
                                                @foreach ($partner->partner_image as $key => $image)
                                                    <div class="col-md-2">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" id="status"
                                        name="partner[status]" required aria-describedby="validationStatusFeedback">
                                        <option value="1"
                                            {{ old('status') == 1 ? 'selected' : (!empty($partner->status) && $partner->status == 1 ? $partner->status : 'selected') }}>
                                            Active</option>
                                        <option value="2"
                                            {{ old('status') == 2 ? 'selected' : (!empty($partner->status) && $partner->status == 2 ? $partner->status : '') }}>
                                            Inactive
                                        </option>
                                    </select>
